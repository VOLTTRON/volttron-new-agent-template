...
class VolttronListerAgent(Agent):
    """
    VolttronListerAgent is an example file that listens to the message bus and prints it to the log.
    """
    ...
    def __init__(self, config_path, **kwargs):
        ...
        if runtime_limit and runtime_limit > 0:
            ...
            _log.info('VolttronListerAgent agent will stop at {}'.format(stop_time))
            self.core.schedule(stop_time, self.core.stop)
        else:
            _log.info('No valid runtime_limit configured; VolttronListerAgent agent will run until manually stopped')
...
def main():
    ...
    """
    try:
        vip_main(VolttronListerAgent, version=__version__)
